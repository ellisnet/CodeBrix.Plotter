// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Legend.Rendering.cs" company="OxyPlot">
...
namespace CodeBrix.Plotter.Legends; //was previously: OxyPlot.Legends;
...
public partial class Legend
{
    ...
    public override PlotterSize GetLegendSize(IRenderContext rc, PlotterSize availableLegendArea)
    {
        ...
        var legendSize = this.MeasureLegends(rc, new PlotterSize(Math.Max(0, availableLegendWidth), Math.Max(0, availableLegendHeight)));

        // Ensure legend size is valid
        legendSize = new PlotterSize(Math.Max(0, legendSize.Width), Math.Max(0, legendSize.Height));
    ...
    public override PlotterRect GetLegendRectangle(PlotterSize legendSize)
    {
        double top = 0;
        double left = 0;
        ...
        return new PlotterRect(left, top, legendSize.Width, legendSize.Height);
    }
    ...
    private void RenderLegend(IRenderContext rc, Series.Series s, PlotterRect rect)
    {
        ...
        double y = rect.Top;
        var maxsize = new PlotterSize(Math.Max(rect.Width - this.LegendSymbolLength - this.LegendSymbolMargin, 0), rect.Height);
        ...
        this.SeriesPosMap.Add(s, new PlotterRect(new ScreenPoint(x, y), textSize));
        double x0 = x;
        switch (actualItemAlignment)
        {
            case HorizontalAlignment.Center:
    ...
    private PlotterSize MeasureLegends(IRenderContext rc, PlotterSize availableSize)
    {
        return this.RenderOrMeasureLegends(rc, new PlotterRect(0, 0, availableSize.Width, availableSize.Height), true);
    }
    ...
    /// <returns>The size of the legend box.</returns>
    private PlotterSize RenderOrMeasureLegends(IRenderContext rc, PlotterRect rect, bool measureOnly = false)
    {
        ...
        if (!string.IsNullOrEmpty(this.LegendTitle))
        {
            ...
            size = new PlotterSize(x + titleSize.Width + this.LegendPadding, top + titleSize.Height);
        }
        ...
        var seriesToRender = new Dictionary<Series.Series, PlotterRect>();
        Action renderItems = () =>
        {
            ...
            foreach (var sr in seriesToRender)
            {
                ...
                var r = new PlotterRect(itemRect.Left, itemRect.Top, Math.Max(rwidth, 0), Math.Max(rheight, 0));
        ...
        foreach (var g in itemGroupNames)
        {
            ...
            PlotterSize groupNameTextSize = new PlotterSize(0, 0);
            ...
            foreach (var s in itemGroup)
            {
                ...
                if (this.LegendOrientation == LegendOrientation.Horizontal)
                {
                    ...
                    if (!measureOnly)
                    {
                        seriesToRender.Add(s, new PlotterRect(rect.Left + x, rect.Top + y, itemWidth, itemHeight));
                    }
                    ...
                    size = new PlotterSize(Math.Max(size.Width, x), Math.Max(size.Height, y + textSize.Height));
                }
                else
                {
                    ...
                    // Update the max width and height of the legend box
                    size = new PlotterSize(Math.Max(size.Width, x + itemWidth), Math.Max(size.Height, y));
                }
        ...
        if (size.Width > 0)
        {
            size = new PlotterSize(size.Width + this.LegendPadding, size.Height);
        }

        if (size.Height > 0)
        {
            size = new PlotterSize(size.Width, size.Height + this.LegendPadding);
        }

        if (size.Width > availableWidth)
        {
            size = new PlotterSize(availableWidth, size.Height);
        }

        if (size.Height > availableHeight)
        {
            size = new PlotterSize(size.Width, availableHeight);
        }

        if (!double.IsNaN(this.LegendMaxWidth) && size.Width > this.LegendMaxWidth)
        {
            size = new PlotterSize(this.LegendMaxWidth, size.Height);
        }

        if (!double.IsNaN(this.LegendMaxHeight) && size.Height > this.LegendMaxHeight)
        {
            size = new PlotterSize(size.Width, this.LegendMaxHeight);
        }
