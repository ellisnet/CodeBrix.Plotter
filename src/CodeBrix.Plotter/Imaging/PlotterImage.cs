// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PlotterImage.cs" company="OxyPlot">
...
public class PlotterImage
{
    ...
    private PlotterColor[,] pixels;

    /// <summary>
    /// Initializes a new instance of the <see cref="PlotterImage" /> class from the specified stream.
    /// </summary>
    /// <param name="s">A stream that provides the image data.</param>
    public PlotterImage(Stream s)
        : this(GetBytes(s))
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="PlotterImage" /> class from a byte array.
    /// </summary>
    /// <param name="bytes">The image bytes.</param>
    public PlotterImage(byte[] bytes)
    {
        this.data = bytes;
    ...
    /// <returns>An <see cref="PlotterImage" /></returns>
    public static PlotterImage Create(
        byte[,] pixels,
        PlotterColor[] palette,
        ImageFormat format,
        ImageEncoderOptions encoderOptions = null)
    {
        var encoder = GetEncoder(format, encoderOptions);
        return new PlotterImage(encoder.Encode(pixels, palette));
    }
    ...
    /// <param name="encoderOptions">The encoder options.</param>
    /// <returns>An <see cref="PlotterImage" /></returns>
    public static PlotterImage Create(PlotterColor[,] pixels, ImageFormat format, ImageEncoderOptions encoderOptions = null)
    {
        var encoder = GetEncoder(format, encoderOptions);
        var image = new PlotterImage(encoder.Encode(pixels));
