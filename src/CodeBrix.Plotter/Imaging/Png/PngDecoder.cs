...
public class PngDecoder : IImageDecoder
{
    ...
    public PlotterImageInfo GetImageInfo(byte[] bytes)
    {
        ...
        return new PlotterImageInfo { Width = width, Height = height, DpiX = dpix, DpiY = dpiy, BitsPerPixel = bitDepth };
    }
    ...
    public PlotterColor[,] Decode(byte[] bytes)
    {
        ...
        var pixels = new PlotterColor[width, height];
        ms.Position = 0;
        for (int y = height - 1; y >= 0; y--)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                pixels[x, y] = PlotterColor.FromArgb(alpha, red, green, blue);
            }
        }
