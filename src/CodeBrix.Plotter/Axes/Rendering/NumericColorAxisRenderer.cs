...
namespace CodeBrix.Plotter.Axes.Rendering; //was previously: OxyPlot.Axes.Rendering;
...
public class NumericColorAxisRenderer<T> : ColorAxisRenderer<T> where T : Axis, INumericColorAxis
{
    ...
    /// <returns>An <see cref="PlotterImage" /> used to render the color axis.</returns>
    private PlotterImage GenerateColorAxisImage(T axis, bool reverse)
    {
        var n = axis.Palette.Colors.Count;
        var buffer = axis.IsHorizontal() ? new PlotterColor[n, 1] : new PlotterColor[1, n];
        for (var i = 0; i < n; i++)
        {
        ...
        return PlotterImage.Create(buffer, ImageFormat.Png);
    }
