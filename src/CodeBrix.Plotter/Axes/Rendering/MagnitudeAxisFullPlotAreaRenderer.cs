...
public class MagnitudeAxisFullPlotAreaRenderer : AxisRendererBase<MagnitudeAxisFullPlotArea>
{
    ...
    public override void Render(MagnitudeAxisFullPlotArea axis, int pass)
    {
        ...
        PlotterRect distancerect = axis.PlotModel.PlotArea.Offset(-magnitudeAxis.MidPoint.X, -magnitudeAxis.MidPoint.Y);
        ...
        if (pass == 1)
        {
            using var _ = this.RenderContext.AutoResetClip(new PlotterRect(axis.ScreenMin, axis.ScreenMax));
    ...
    private void RenderTickArc(MagnitudeAxisFullPlotArea axis, AngleAxis angleAxis, double x, PlotterPen pen, double startAngle, double endAngle)
    {
