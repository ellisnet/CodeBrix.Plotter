...
public class HeatMapSeries : XYAxisSeries
{
    ...
    public override void Render(IRenderContext rc)
    {
        ...
        var rect = new PlotterRect(s00, s11);
        ...
        else
        {
            ...
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    var rectrect = new PlotterRect(pointa, pointb);

                    rc.DrawRectangle(rectrect, rectcolor, PlotterColors.Undefined, 0, this.EdgeRenderingMode);
                }
            }
        }
    ...
    protected virtual void RenderLabels(IRenderContext rc, PlotterRect rect)
    {
        int m = this.Data.GetLength(0);
        int n = this.Data.GetLength(1);
        ...
        for (int i = 0; i < m; i++)
        {
            for (int j = 0; j < n; j++)
            {
                ...
                var hsv = color.ToHsv();
                var textColor = hsv[2] > 0.6 ? PlotterColors.Black : PlotterColors.White;
    ...
    private void UpdateImage()
    {
        ...
        int m = this.Data.GetLength(0);
        int n = this.Data.GetLength(1);
        var buffer = swapXY ? new PlotterColor[n, m] : new PlotterColor[m, n];
        for (int i = 0; i < m; i++)
        {
        ...
        this.image = PlotterImage.Create(buffer, ImageFormat.Png);
    }
}
