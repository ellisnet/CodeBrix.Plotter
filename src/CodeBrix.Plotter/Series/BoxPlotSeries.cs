...
public class BoxPlotSeries : XYAxisSeries
{
    ...
    /// <summary>
    /// Gets or sets the a custom polygon outline for the outlier markers. Set <see cref="OutlierType" /> to <see cref="CodeBrix.Plotter.MarkerType.Custom" /> to use this property.
    /// </summary>
    ...
    public override void Render(IRenderContext rc)
    {
        ...
        foreach (var item in this.ActualItems)
        {
            ...
            else
            {
                ...
                {
                    var ellipseRect = new PlotterRect(
                        mc.X - this.MedianPointSize,
                        mc.Y - this.MedianPointSize,
                        this.MedianPointSize * 2,
                        this.MedianPointSize * 2);
                    rc.DrawEllipse(ellipseRect, fillColor, PlotterColors.Undefined, 0, this.EdgeRenderingMode);
                }
            }
    ...
    public override void RenderLegend(IRenderContext rc, PlotterRect legendBox)
    {
        ...
        if (this.ShowBox)
        {
            // box
            rc.DrawRectangle(
                new PlotterRect(xmid - halfBoxWidth, ytop, 2 * halfBoxWidth, ybottom - ytop),
                fillColor,
                strokeColor,
                LegendStrokeThickness,
                this.EdgeRenderingMode.GetActual(EdgeRenderingMode.PreferGeometricAccuracy));
        ...
        else
        {
            var ellipseRect = new PlotterRect(
                ...
                this.MedianPointSize * 2,
                this.MedianPointSize * 2);
            rc.DrawEllipse(ellipseRect, fillColor, PlotterColors.Undefined, 0, this.EdgeRenderingMode.GetActual(EdgeRenderingMode.PreferGeometricAccuracy));
        }
    }
    ...
    private PlotterRect GetBoxRect(BoxPlotItem item)
    {
        var halfBoxWidth = this.BoxWidth * 0.5;
