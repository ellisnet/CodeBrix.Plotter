...
{
    private readonly Stack<PlotterRect> clipStack = new Stack<PlotterRect>();
    ...
    /// <inheritdoc/>
    public sealed override void PushClip(PlotterRect clippingRectangle)
    {
        if (this.clipStack.Count > 0)
        {
    ...
    /// <remarks>
    /// Calls to this method must always be balanced by a call to <see cref="ResetClip"/> before calling <see cref="SetClip(PlotterRect)"/> again.
    /// </remarks>
    protected abstract void SetClip(PlotterRect clippingRectangle);
}
