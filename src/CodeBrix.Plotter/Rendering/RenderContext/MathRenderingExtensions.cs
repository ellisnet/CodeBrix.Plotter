...
public static class MathRenderingExtensions
{
    ...
    public static PlotterSize DrawMathText(
        ...
        bool measure)
    {
        ...
        if (text.Contains("^{") || text.Contains("_{"))
        {
            ...
            return measure ? size : PlotterSize.Empty;
        }
    ...
    /// <param name="fontWeight">The font weight.</param>
    /// <returns>The size of the text.</returns>
    public static PlotterSize MeasureMathText(
        this IRenderContext rc, string text, string fontFamily, double fontSize, double fontWeight)
    {
        if (text.Contains("^{") || text.Contains("_{"))
        {
            return InternalDrawMathText(rc, 0, 0, 0, 0, text, PlotterColors.Black, fontFamily, fontSize, fontWeight, true, 0.0);
        }
    ...
    /// <returns>The size of the text.</returns>
    private static PlotterSize InternalDrawMathText(
        IRenderContext rc,
        double x,
        double y,
        double dx,
        ...
        double angle)
    {
        ...
        Func<double, double, string, double, PlotterSize> drawText = (xb, yb, text, fSize) =>
            {
                if (!measureOnly)
                {
                ...
                return new PlotterSize(flatSize.Width, flatSize.Height);
            };
        ...
        return new PlotterSize(maximumX - minimumX, maximumY - minimumY);
    }
}
