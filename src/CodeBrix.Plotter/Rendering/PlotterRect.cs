// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PlotterRect.cs" company="OxyPlot">
...
/// Describes the width, height, and point origin of a rectangle.
/// </summary>
public struct PlotterRect : IFormattable, IEquatable<PlotterRect>
{
    /// <summary>
    /// Gets an infinitely large <see cref="PlotterRect"/> starting at (0,0).
    /// </summary>
    public static readonly PlotterRect Everything = new PlotterRect(0, 0, double.PositiveInfinity, double.PositiveInfinity);
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="PlotterRect" /> structure that has the specified x-coordinate, y-coordinate, width, and height.
    /// </summary>
    ...
    public PlotterRect(double left, double top, double width, double height)
    {
        if (width < 0)
        {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="PlotterRect" /> struct that is exactly large enough to contain the two specified points.
    /// </summary>
    ...
    public PlotterRect(ScreenPoint p0, ScreenPoint p1)
        : this(Math.Min(p0.X, p1.X), Math.Min(p0.Y, p1.Y), Math.Abs(p1.X - p0.X), Math.Abs(p1.Y - p0.Y))
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="PlotterRect"/> struct by location and size.
    /// </summary>
    /// <param name="p0">The location.</param>
    /// <param name="size">The size.</param>
    public PlotterRect(ScreenPoint p0, PlotterSize size)
    ...
    public static PlotterRect Create(double x0, double y0, double x1, double y1)
    {
        return new PlotterRect(Math.Min(x0, x1), Math.Min(y0, y1), Math.Abs(x1 - x0), Math.Abs(y1 - y0));
    }
    ...
    /// <summary>
    /// Determines whether this instance and another specified <see cref="T:PlotterRect" /> object have the same value.
    /// </summary>
    ...
    public bool Equals(PlotterRect other)
    {
    ...
    /// <returns>The expanded/shrunk <see cref="PlotterRect" />.</returns>
    public PlotterRect Inflate(double dx, double dy)
    {
        return new PlotterRect(this.left - dx, this.top - dy, this.width + (dx * 2), this.height + (dy * 2));
    }
    ...
    /// <returns>The inflated <see cref="PlotterRect" />.</returns>
    public PlotterRect Inflate(PlotterThickness t)
    {
        return new PlotterRect(this.left - t.Left, this.top - t.Top, this.width + t.Left + t.Right, this.height + t.Top + t.Bottom);
    }

    /// <summary>
    /// Intersects this <see cref="PlotterRect"/> with another <see cref="PlotterRect"/>.
    /// </summary>
    /// <param name="rect">The other <see cref="PlotterRect"/>.</param>
    /// <returns>The intersection between this <see cref="PlotterRect"/> and the other <see cref="PlotterRect"/>.</returns>
    /// <remarks>If the two rectangles don't intersect, this returns an empty <see cref="PlotterRect"/>.</remarks>
    public PlotterRect Intersect(PlotterRect rect)
    {
        ...
        {
            return new PlotterRect();
        }

        return new PlotterRect(left, top, right - left, bottom - top);
    }
    ...
    /// </summary>
    /// <param name="t">The thickness to apply to the rectangle.</param>
    /// <returns>The deflated <see cref="PlotterRect" />.</returns>
    public PlotterRect Deflate(PlotterThickness t)
    {
        return new PlotterRect(this.left + t.Left, this.top + t.Top, Math.Max(0, this.width - t.Left - t.Right), Math.Max(0, this.height - t.Top - t.Bottom));
    }
    ...
    /// <returns>The moved <see cref="PlotterRect" />.</returns>
    public PlotterRect Offset(double offsetX, double offsetY)
    {
        return new PlotterRect(this.left + offsetX, this.top + offsetY, this.width, this.height);
    }
    ...
    public PlotterRect Clip(PlotterRect clipRect)
    {
