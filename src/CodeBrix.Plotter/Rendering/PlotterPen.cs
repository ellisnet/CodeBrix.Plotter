// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PlotterPen.cs" company="OxyPlot">
...
/// Describes a pen in terms of color, thickness, line style and line join type.
/// </summary>
public class PlotterPen
{
    /// <summary>
    /// Initializes a new instance of the <see cref="PlotterPen" /> class.
    /// </summary>
    ...
    public static PlotterPen Create(
        PlotterColor color,
        double thickness,
        LineStyle lineStyle = LineStyle.Solid,
        LineJoin lineJoin = LineJoin.Miter)
    {
        if (color.IsInvisible() || lineStyle == LineStyle.None || Math.Abs(thickness) <= 0)
        {
            return null;
        }

        return new PlotterPen(color, thickness, lineStyle, lineJoin);
    }
