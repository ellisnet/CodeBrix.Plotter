...
public partial class PlotModel
{
    ...
    void IPlotModel.Render(IRenderContext rc, PlotterRect rect)
    {
    ...
    /// <param name="rc">The rendering context.</param>
    /// <param name="rect">The plot bounds.</param>
    protected virtual void RenderOverride(IRenderContext rc, PlotterRect rect)
    {
        lock (this.SyncRoot)
        {
            ...
            try
            {
                ...
                if (this.lastPlotException != null)
                {
                    ...
                    // This could happen when CodeBrix.Plotter is given invalid input data.
                    ...
                    this.RenderErrorMessage(rc, string.Format("CodeBrix.Plotter exception: {0}", this.lastPlotException.Message), errorMessage);
                    return;
                }
            ...
            catch (Exception exception)
            {
                ...
                this.RenderErrorMessage(rc, string.Format("CodeBrix.Plotter exception: {0}", exception.Message), errorMessage);
            }
            finally
            {
    ...
    private bool AdjustPlotMargins(IRenderContext rc)
    {
        ...
        var desiredMargin = new PlotterThickness();

        void IncludeInMargin(double size, AxisPosition borderPosition)
        {
            desiredMargin = borderPosition switch
            {
                AxisPosition.Bottom => new PlotterThickness(desiredMargin.Left, desiredMargin.Top, desiredMargin.Right, Math.Max(desiredMargin.Bottom, size)),
                AxisPosition.Left => new PlotterThickness(Math.Max(desiredMargin.Left, size), desiredMargin.Top, desiredMargin.Right, desiredMargin.Bottom),
                AxisPosition.Right => new PlotterThickness(desiredMargin.Left, desiredMargin.Top, Math.Max(desiredMargin.Right, size), desiredMargin.Bottom),
                AxisPosition.Top => new PlotterThickness(desiredMargin.Left, Math.Max(desiredMargin.Top, size), desiredMargin.Right, desiredMargin.Bottom),
                _ => desiredMargin,
            };
        }
        ...
        currentMargin = new PlotterThickness(
    ...
    private PlotterSize MeasureTitles(IRenderContext rc)
    {
        ...
        return new PlotterSize(width, height);
    }
    ...
    private void RenderBackgrounds(IRenderContext rc)
    {
        ...
        {
            rc.DrawRectangle(this.PlotArea, this.PlotAreaBackground, PlotterColors.Undefined, 0, this.EdgeRenderingMode);
        }

        foreach (var s in this.Series.Where(s => s.IsVisible && s is XYAxisSeries && s.Background.IsVisible()).Cast<XYAxisSeries>())
        {
            rc.DrawRectangle(s.GetScreenRectangle(), s.Background, PlotterColors.Undefined, 0, this.EdgeRenderingMode);
        }
    }
    ...
    {
        var previousClippingRect = PlotterRect.Everything;
        ...
        foreach (var plotElement in plotElements)
        {
            ...
            {
                if (!previousClippingRect.Equals(PlotterRect.Everything))
                {
                    rc.PopClip();
                    previousClippingRect = PlotterRect.Everything;
                }

                if (!currentClippingRect.Equals(PlotterRect.Everything))
                {
    ...
    private void RenderTitle(IRenderContext rc)
    {
        ...
        if (this.ClipTitle)
        {
            maxSize = new PlotterSize(this.TitleArea.Width * this.TitleClippingLength, double.MaxValue);
        }
    ...
    {
        var plotAndAxisArea = new PlotterRect(
        ...
        if (titleSize.Height > 0)
        {
            ...
            plotAndAxisArea = new PlotterRect(plotAndAxisArea.Left, plotAndAxisArea.Top + titleHeight, plotAndAxisArea.Width, Math.Max(0, plotAndAxisArea.Height - titleHeight));
        }
        ...
        if (this.IsLegendVisible)
        {
            ...
            PlotterSize maxLegendSize = new PlotterSize(0, 0);
                ...
                (l.LegendPosition == LegendPosition.LeftTop || l.LegendPosition == LegendPosition.LeftMiddle || l.LegendPosition == LegendPosition.LeftBottom))))
            {
                ...
                var lsiz = legend.GetLegendSize(rc, new PlotterSize(availableLegendWidth, availableLegendHeight));
                legend.LegendSize = lsiz;
                maxLegendSize = new PlotterSize(maxLegendSize.Width > lsiz.Width ? maxLegendSize.Width : lsiz.Width, maxLegendSize.Height > lsiz.Height ? maxLegendSize.Height : lsiz.Height);
            ...
            {
                plotArea = new PlotterRect(plotArea.Left + maxLegendSize.Width + legendMargin, plotArea.Top, Math.Max(0, plotArea.Width - (maxLegendSize.Width + legendMargin)), plotArea.Height);
            }

            maxLegendSize = new PlotterSize(0, 0);
            legendMargin = 0;
            ...
            if (maxLegendSize.Width > 0 || maxLegendSize.Height > 0)
            {
                plotArea = new PlotterRect(plotArea.Left, plotArea.Top, Math.Max(0, plotArea.Width - (maxLegendSize.Width + legendMargin)), plotArea.Height);
            }
            ...
            if (maxLegendSize.Width > 0 || maxLegendSize.Height > 0)
            {
                plotArea = new PlotterRect(plotArea.Left, plotArea.Top + maxLegendSize.Height + legendMargin, plotArea.Width, Math.Max(0, plotArea.Height - (maxLegendSize.Height + legendMargin)));
            }
            ...
            if (maxLegendSize.Width > 0 || maxLegendSize.Height > 0)
            {
                plotArea = new PlotterRect(plotArea.Left, plotArea.Top, plotArea.Width, Math.Max(0, plotArea.Height - (maxLegendSize.Height + legendMargin)));
            }
            ...
            foreach (var legend in this.Legends.Where(l => l.LegendPlacement == LegendPlacement.Inside && l.IsLegendVisible))
            {
                ...
                legend.LegendSize = legend.GetLegendSize(rc, new PlotterSize(availableLegendWidth, availableLegendHeight));
            }
        }
        ...
        {
            plotArea = new PlotterRect(plotArea.Left, plotArea.Top, plotArea.Width, 1);
        }

        if (plotArea.Width < 0)
        {
            plotArea = new PlotterRect(plotArea.Left, plotArea.Top, 1, plotArea.Height);
        }
        ...
        switch (this.TitleHorizontalAlignment)
        {
            case TitleHorizontalAlignment.CenteredWithinView:
                this.TitleArea = new PlotterRect(
