...
public class PolarHeatMapSeries : XYAxisSeries
{
    ...
    public void RenderDynamic(IRenderContext rc, PlotModel model)
    {
        ...
        {
            this.pixels = new PlotterColor[width, height];
        }
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                if (ii >= 0 && ii < m && jj >= 0 && jj < n)
                {
                    ...
                    p[x, y] = PlotterColor.FromAColor(160, this.ColorAxis.GetColor(value));
                }
                else
                {
                    // outside the range of the Data array
                    p[x, y] = PlotterColors.Transparent;
                }
            }
        }

        // Create the PNG image
        this.image = PlotterImage.Create(p, ImageFormat.Png);
    ...
    public void RenderFixed(IRenderContext rc, PlotModel model)
    {
            ...
            for (int yi = 0; yi < height; yi++)
            {
                for (int xi = 0; xi < width; xi++)
                {
                    ...
                    if (ii >= 0 && ii < m && jj >= 0 && jj < n)
                    {
                        ...
                        // use the color axis to get the color
                        p[xi, yi] = PlotterColor.FromAColor(160, this.ColorAxis.GetColor(value));
                    }
                    else
                    {
                        // outside the range of the Data array
                        p[xi, yi] = PlotterColors.Transparent;
                    }
                }
            }
        ...
        if (this.PlotModel.PlotType != PlotType.Polar)
        {
            ...
            dest = new PlotterRect(topleft.X, topleft.Y, bottomright.X - topleft.X, bottomright.Y - topleft.Y);
        }
        else
        {
            ...
            dest = new PlotterRect(left.X, top.Y, right.X - left.X, bottom.Y - top.Y);
        }
