...
public class MatrixSeries : XYAxisSeries
{
    ...
    public MatrixSeries()
    {
        ...
        this.GridColor = PlotterColors.LightGray;
        this.BorderColor = PlotterColors.Gray;
        this.NotZeroColor = PlotterColors.Black;
    ...
    public override void Render(IRenderContext rc)
    {
        ...
        if (this.image == null)
        {
            var pixels = new PlotterColor[n, m];
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    pixels[j, i] = Math.Abs(this.Matrix[i, j]) <= this.ZeroTolerance ? PlotterColors.Transparent : this.NotZeroColor;
                }
            }

            this.image = PlotterImage.Create(pixels, ImageFormat.Png);
        }
