...
public class TileMapAnnotation : Annotation
{
    ...
    /// The image cache.
    /// </summary>
    private readonly Dictionary<string, PlotterImage> images = new Dictionary<string, PlotterImage>();
    ...
    public TileMapAnnotation()
    {
        ...
        this.UserAgent = "PlotterPlotExampleLibrary";
    }
    ...
    public override void Render(IRenderContext rc)
    {
        ...
        for (var x = (int)xmin; x < xmax; x++)
        {
            for (var y = (int)ymin; y < ymax; y++)
            {
                ...
                var r = PlotterRect.Create(s00.X, s00.Y, s11.X, s11.Y);
        ...
        rc.DrawRectangle(
            new PlotterRect(p.X - textSize.Width - 2, p.Y - textSize.Height - 2, textSize.Width + 4, textSize.Height + 4),
            PlotterColor.FromAColor(200, PlotterColors.White),
            PlotterColors.Undefined,
            0,
    ...
    private PlotterImage GetImage(string uri, bool asyncLoading)
    {
        PlotterImage img;
    ...
    /// <summary>
    /// The client used to fetch map tiles.
    /// </summary>
    /// <remarks>
    /// Upstream used HttpWebRequest with the Begin/End callback pattern. That
    /// API is obsolete on modern .NET (SYSLIB0014), so this is an HttpClient,
    /// held statically because HttpClient is designed to be shared and reused.
    /// </remarks>
    private static readonly HttpClient TileClient = new HttpClient();
    ...
    /// <param name="uri">The URI.</param>
    /// <returns>The image</returns>
    private PlotterImage Download(string uri)
    {
        PlotterImage img = null;
        ...
        try
        {
            ...
            using var response = TileClient.Send(request);
            ...
            img = new PlotterImage(ms.ToArray());
            this.images[uri] = img;
        }
        catch (Exception e)
        {
    ...
    private void BeginDownload()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                using var response = await TileClient.SendAsync(request).ConfigureAwait(false);
                response.EnsureSuccessStatusCode();
    ...
    private void DownloadCompleted(string uri, Stream result)
    {
        ...
        var img = new PlotterImage(buffer);
        this.images[uri] = img;
