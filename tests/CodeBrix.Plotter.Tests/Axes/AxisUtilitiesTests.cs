...
public class AxisUtilitiesTests
{
    ...
    {
        AxisUtilities.CreateTickValues(1, 100, 0.000001).Count.Should().Be(1000);
        AxisUtilities.CreateTickValues(1, 100, 0.000001, 100).Count.Should().Be(100);
        AxisUtilities.CreateTickValues(1, 100, 0.000001, 0).Count.Should().Be(0);
    }
    ...
    public void TestCreateTickValues()
    {
        ...
        void Test(double from, double to, double step, IList<double> expected)
        {
            ...
            actual.Should().Equal(expected, (a, e) => System.Math.Abs(a - e) < step * 1e-7);
        }
    }
    ...
    [Theory]
    [InlineData(1e-100, .2e-100)]
    [InlineData(2e-100, .5e-100)]
    [InlineData(5e-100, 1e-100)]
    [InlineData(1, 0.2)]
    [InlineData(2, 0.5)]
    [InlineData(5, 1)]
    [InlineData(20, 5)]
    [InlineData(1e100, .2e100)]
    [InlineData(2e100, .5e100)]
    [InlineData(5e100, 1e100)]
    public void CalculateMinorInterval(double majorInterval, double expectedMinorInterval)
    {
        AxisUtilities.CalculateMinorInterval(majorInterval).Should().BeApproximately(expectedMinorInterval, expectedMinorInterval * 1e-10);
#if DEBUG
        AxisUtilities.CalculateMinorInterval2(majorInterval).Should().BeApproximately(expectedMinorInterval, expectedMinorInterval * 1e-10);
#endif
    }
