...
public static class PngAssert
{
    ...
    public static void AreEqual(string expected, string actual, string message, string output)
    {
        ...
        var differences = 0;
        var differenceImage = new PlotterColor[w, h];
        for (var i = 0; i < h; i++)
        {
            for (var j = 0; j < w; j++)
            {
                ...
                {
                    differences++;
                    differenceImage[j, i] = PlotterColors.Red;
                }
                else
                {
    ...
    private static PlotterColor[,] LoadImage(string path)
    {
        if (!File.Exists(path))
        {
            return null;
        ...
        var ret = new PlotterColor[bitmap.Width, bitmap.Height];
